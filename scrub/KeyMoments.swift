        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            ForEach(0..<videoVM.urls.count, id: \.self) { index in
                ZStack {
                    ...
                    HStack(spacing: 4) {
                        Image(
                            index == videoVM.currentURLIndex
                                && videoVM.isPlaying ? .playing : .play
                        )
                        .resizable()
                ...
                .onTapGesture {
                    ...
                    let avAsset = AVAsset(
                        url: URL(string: videoVM.urls[index])!
                    )
                    Task {
                        videoVM.totalDuration =
                            (try? await avAsset.load(.duration))?.seconds
